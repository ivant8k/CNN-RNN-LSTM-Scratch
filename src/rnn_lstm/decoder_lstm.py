...
from rnn_lstm.layers.lstm import LSTM
...
class DecoderLSTM:
    def __init__(
        ...
        # Recurrent Layers
        self.lstm_layers: List[LSTM] = []
        for i in range(num_layers):
            if i == 0:
                self.lstm_layers.append(LSTM(input_dim=embed_dim, hidden_size=hidden_size))
            else:
                self.lstm_layers.append(LSTM(input_dim=hidden_size, hidden_size=hidden_size))
    ...
    def set_model_params(self, params: Dict[str, Any]) -> None:
        ...
        for i in range(self.num_layers):
            self.lstm_layers[i].set_weights(params[f'lstm_{i}'])
